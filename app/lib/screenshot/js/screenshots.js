...
screenshots={
    ...
    init:function(url,resolution ){
        ...
        oBut.onclick=function(){
            ...
            var reg_filetype = new RegExp("{{btype}}", "g");
            url = url.replace(reg_filetype, oformat);

            var oUrl=url+"&resolutions="+ore;
            console.log(oUrl);
            //出现浏览器下载框
    ...
    showCoords:function(c){
        ...
        //console.log(leftTopX,leftTopY,bottomRightX,bottomRightY);
        while (leftTopX > 180) {
            leftTopX = leftTopX - 360;
        }
        while (leftTopX < -180) {
            leftTopX = leftTopX + 360;
        }

        while (bottomRightX > 180) {
            bottomRightX = bottomRightX - 360;
        }
        while (bottomRightX < -180) {
            bottomRightX = bottomRightX + 360;
        }

...
var timeLine = {
    ...
    DateTimeChange: function () {
        $(timeLine._el.container).trigger("DateTimeChange", [
            timeLine.options.moment_select
        ]);
    },
    ...
    _init_Year_div: function (controller_div) {
        ...
        var addYearSvg = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "svg"
        );
        addYearDiv.appendChild(addYearSvg);

        var addYearPath = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "path"
        );
        ...
        var minusYearSvg = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "svg"
        );
        minusYearDiv.appendChild(minusYearSvg);

        var minusYearPath = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "path"
        );
    ...
    _init_Month_div: function (controller_div) {
        ...
        var addMonthSvg = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "svg"
        );
        addMonthDiv.appendChild(addMonthSvg);

        var addMonthPath = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "path"
        );
        ...
        addMonth_input.setAttribute(
            "value",
            timeLine.options.moment_select.format("MM")
        );
        ...
        var minusMonthSvg = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "svg"
        );
        minusMonthDiv.appendChild(minusMonthSvg);

        var minusMonthPath = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "path"
        );
    ...
    _init_Day_div: function (controller_div) {
        ...
        var addDaySvg = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "svg"
        );
        addDayDiv.appendChild(addDaySvg);

        var addDayPath = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "path"
        );
        ...
        addDay_input.setAttribute(
            "value",
            timeLine.options.moment_select.format("DD")
        );
        ...
        var minusDaySvg = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "svg"
        );
        ...
        var minusDayPath = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "path"
        );
    ...
    _init_Hour_div: function (controller_div) {
        ...
        var addHourSvg = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "svg"
        );
        addHourDiv.appendChild(addHourSvg);

        var addHourPath = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "path"
        );
        // var addYearPath = document.createElement("path");
        ...
        addHour_input.setAttribute(
            "value",
            timeLine.options.moment_select.format("HH")
        );
        ...
        var minusHourSvg = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "svg"
        );
        minusHourDiv.appendChild(minusHourSvg);

        var minusHourPath = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "path"
        );
    ...
    _init_sp_div: function (controller_div) {
        ...
        var background_svg = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "svg"
        );
    ...
    _init_pre_div: function (controller_div) {
        ...
        var prePath = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "path"
        );
    ...
    _preClickFunc: function () {
        var selectMode =
            timeLine.options.mode_list[timeLine.options.select_modeindex];
        switch (selectMode) {
            case "year":
                {
...
// todo 20180425 修改添加 版权部分 --img路径待议 liuyp
        ...
        /*var hide_img = document.createElement("img");
        hide_img.style.width = "155px";
        hide_img.style.float= "right";
       // hide_img.style.float= "right";

        hide_img.id = "hide_img";
        hide_img.src = "/publics/trademark.png";
      //  hide_div.setAttribute("class", "hide_div");
        //  hide_div.style.background = timeLine.options.default_bg_color;
        hide_div.appendChild(hide_img);*/


        var hideSvg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
        hideSvg.style.width = "25px";
        hideSvg.style.float= "left";
        hide_div.appendChild(hideSvg);

        var x_location = "5";
        var nextPath_1 = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "rect"
        );
        ...
        var nextPath_2 = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "rect"
        );
        ...
        var nextPath_3 = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "rect"
        );
    ...
    _init_TimeChange_div: function (controller_div) {
        //添加 模式选项
    ...
    _getTransByTime: function (addminus_num, timeUnit) {
        ...
        //获取当前选择时间
        var selectUnit =
            timeLine.options.mode_unit[timeLine.options.select_modeindex];
        var selectUnit_pix =
            timeLine.options.mode_timespan[timeLine.options.select_modeindex];
        var selectMoment_str = timeLine.options.moment_select
            .utc()
            .format("YYYYMMDDHHmmss");
        var old_moment = moment
            .utc(selectMoment_str, "YYYYMMDDHHmmss")
            .add(addminus_num, timeUnit);
        var return_trans =
            old_moment.diff(timeLine.options.moment_select, selectUnit, true) *
            selectUnit_pix;
        //根据当前 单位
        return -return_trans;
    },
    ...
    _clearTimeLine: function () {
        ...
        if (_canvas) {
            ...
            ctx.clearRect(
                0,
                0,
                this.options.timeline_width,
                this.options.timeline_height
            );
        }
    ...
    _getDateMouseIn: function (mouse_x) {
        //获取当前开始时间
        var time_begin_str = timeLine.options.moment_begin
            .utc()
    ...
    _init_beginTime: function () {
        ...
        var unitNum_after = parseFloat(width_after / selectmodetimespan);
        var moment_begin = moment.utc(
            this.options.moment_select.format("YYYY-MM-DD HH:mm:ss"),
            "YYYY-MM-DD HH:mm:ss"
        );
    ...
    _draw_timeline: function (begin_date, end_date, select_date, unit_str, unit_pix) {
        ...
        //计算开始绘制时间
        var moment_draw = moment
            .utc(begin_date.format("YYYY-MM-DD HH:mm:ss"), "YYYY-MM-DD HH:mm:ss")
            .startOf(unit_str);
        ...
        while (moment_draw < end_date) {
            ...
            if (timeLine._getArcMode(moment_draw)) {
                timeLine._canvas_arc(
                    "white",
                    _line_x,
                    timeLine.options.timeline_height_top,
                    7
                );
                timeLine._canvas_line(
                    "white",
                    _line_x,
                    3,
                    trans - unit_pix,
                    timeLine.options.timeline_height
                );
                var txtStr = timeLine._getDateStr(moment_draw);
                timeLine._canvas_txt(
                    "white",
                    txtStr,
                    _line_x + 5,
                    timeLine.options.timeline_height - 3
                );
            }
            ...
            if (timeLine._getLongLineMode(moment_draw)) {
                timeLine._canvas_line(
                    "white",
                    _line_x,
                    timeLine.options.timeline_height_top - 20,
                    _line_x,
                    timeLine.options.timeline_height_top
                );
            } else {
                ...
                timeLine._canvas_dashed_line(
                    "white",
                    _line_x,
                    timeLine.options.timeline_height_top - 10,
                    _line_x,
                    timeLine.options.timeline_height_top
                );
            }
        ...
        if (begin_date.isBefore(select_date) && end_date.isAfter(select_date)) {
            var selectTrans =
                (1 - begin_date.diff(select_date, unit_str, true) - 1) * unit_pix;
            //timeLine._canvas_arc("red", selectTrans - 7, timeLine.options.timeline_height_top, 7);
            // timeLine._canvas_line("#EEEEEE", selectTrans, 0, selectTrans, 73);
            timeLine._canvas_image_1(selectTrans, 51);
            // timeLine._canvas_image(selectTrans, 51);
        }
    },

    _getDataShowInfo: function (rect_height,
        rect_y,
        datainfo,
        begin_date_str,
        end_date_str,
        ...
        if (is_Draw) {
            ...
            if (
                begin_date_str <= datainfo.begintime &&
                ...
                datainfo.begintime <= begin_date_str &&
    ...
    //canvas 画线
    _canvas_line: function (fillcolor,
        begin_point_x,
        begin_point_y,
        end_point_x,
        end_point_y) {
    ...
    //canvas 绘制 断点线
    _canvas_dashed_line: function (fillcolor,
        begin_point_x,
        begin_point_y,
        end_point_x,
        end_point_y) {
    ...
    //canvas 画矩形
    _canvas_rect: function (fillcolor,
        begin_point_x,
        begin_point_y,
        rect_width,
        rect_heigth) {
    ...
    //canvas 绘制图案
    _canvas_image: function (begin_x, begin_y) {
        var _canvas = timeLine._el.timecanvas;
        if (_canvas) {
            var context = _canvas.getContext("2d");
            var img = new Image();
            img.src =
                "data:image/png;base64,";
            img.onload = function () {
                context.drawImage(img, begin_x - 10, begin_y, 20, 15);
            };
        }
    },

    _canvas_image_1: function (begin_x, begin_y) {
        var _canvas = timeLine._el.timecanvas;
        if (_canvas) {
            var context = _canvas.getContext("2d");
            var img = new Image();
            img.src =
                "data:image/png;base64,";
            // "data:image/png;base64,";

            img.onload = function () {
                context.drawImage(img, begin_x - 15, begin_y - 48, 30, 48);
            };
        }
    },
    _resizeScreen: function () {
        timeLine.options.default_width = document.body.offsetWidth;
        timeLine.options.timeline_width =
            timeLine.options.default_width - timeLine.options.controller_width_sp;
    ...
    setSelectMoment: function (newDate) {
        var old_moment = moment.utc(
            timeLine.options.moment_select.format("YYYYMMDDHHmmss"),
            "YYYYMMDDHHmmss"
        );
        var isReset = false;
    ...
    _dataConvert: function (datainfo) {
        ...
        for (var i = 0; i < _length; i++) {
            ...
            var endTime = datainfo.datainfolist[i].endtime;
            if (!beginTime || !endTime) {
                continue;
            }
            var yearItem = datainfo.datainfolist[i];
            var monthItem = datainfo.datainfolist[i];
            var dayItem = datainfo.datainfolist[i];
            if (beginTime.substr(0, 6) !== endTime.substr(0, 6)) {
                yearItem = {
                ...
                yearItem = {
                    "beginTime": beginTime.substr(0, 6),
                    "endTime": moment.utc(beginTime.substr(0, 6), "YYYYMM").add(1.0, "month").format("YYYYMM")
                };
            }
            if (beginTime.substr(0, 8) !== endTime.substr(0, 8)) {
                monthItem = {
                    "beginTime": beginTime.substr(0, 8),
                    "endTime": endTime.substr(0, 8)
                };
            } else {
                monthItem = {
                    "beginTime": beginTime.substr(0, 8),
                    "endTime": moment.utc(beginTime.substr(0, 8), "YYYYMMDD").add(1.0, "day").format("YYYYMMDD")
                };
            }
            if (beginTime.substr(0, 10) !== endTime.substr(0, 10)) {
                ...
                dayItem = {
                    "beginTime": beginTime.substr(0, 10),
                    "endTime": moment.utc(beginTime.substr(0, 10), "YYYYMMDDHH").add(1.0, "hour").format("YYYYMMDDHH")
                };
            }
            YearModeList.push(yearItem);

            MonthModeList.push(monthItem);

            DayModeList.push(dayItem);
        ...
        //年处理
        YearModeList.forEach(function (yearitem) {
            var yearBegin = yearitem.beginTime;
            var yearEnd = yearitem.endTime;
            var BeginYear_MomentStr = moment.utc(yearBegin, "YYYYMM").format("YYYYMMDDHHmmss");
            var EndYear_MomentStr = moment.utc(yearEnd, "YYYYMM").format("YYYYMMDDHHmmss");
            monthList.push({
        ...
        //月处理
        MonthModeList.forEach(function (monthitem) {
            var monthBegin = monthitem.beginTime;
            var monthEnd = monthitem.endTime;
            var BeginMonth_MomentStr = moment.utc(monthBegin, "YYYYMMDD").format("YYYYMMDDHHmmss");
            var EndMonth_MomentStr = moment.utc(monthEnd, "YYYYMMDD").format("YYYYMMDDHHmmss");
            dayList.push({
        ...
        //日处理
        DayModeList.forEach(function (dayitem) {
            var dayBegin = dayitem.beginTime;
            var dayEnd = dayitem.endTime;
            var BeginDay_MomentStr = moment.utc(dayBegin, "YYYYMMDDHH").format("YYYYMMDDHHmmss");
            var EndDay_MomentStr = moment.utc(dayEnd, "YYYYMMDDHH").format("YYYYMMDDHHmmss");
            hourList.push({
        ...
        hourList = _.uniqBy(hourList, "begintime");
        dayList = _.uniqBy(dayList, "begintime");
        monthList = _.uniqBy(monthList, "begintime");

        DataInfoJson.month = monthList;
        DataInfoJson.day = dayList;
        DataInfoJson.hour = hourList;

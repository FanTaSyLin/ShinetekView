...
function Palette() {
    ...
    this.init_palette = function (m_DivID, m_Json) {
        ...
        if (json_CheckFunc()) {
            ...
            try {
                ...
                console.log("调色板设置异常，请检查配置");
            }
        }
    };

    this.destroy_palette = function (m_DivID) {
        try {
            self.target_div = document.getElementById(m_DivID);
            self.target_div.innerHTML = "";
        } catch (err) {}
    };
    ...
    var json_CheckFunc = function () {
        ...
        if (!self.m_BeginJson.palette_mode) {
    ...
    var init_gradient_canvas = function () {
        ...
        if (gradient_Num > 1) {
            for (var i = 0; i < gradient_Num; i++) {
                gradient_data_i = gradient_data[i][1];
                RGB_I = "rgb(" + gradient_data_i[0] + ", " + gradient_data_i[1] + ", " + gradient_data_i[2] + ")";
                grd.addColorStop(1 / (gradient_Num - 1) * i, RGB_I);
            }
        } else {
            gradient_data_i = gradient_data[0][1];
            RGB_I = "rgb(" + gradient_data_i[0] + ", " + gradient_data_i[1] + ", " + gradient_data_i[2] + ")";
    ...
    var init_range = function () {
        ...
        var m_width = 19;
        if (range_num > 0 && m_MaxWidth < 2) {
            m_width = 221 / range_num;
        }
        if (range_num > 0 && m_MaxWidth > 2) {
            var _width = (m_MaxWidth - 2) / range_num;
            if (_width > 0) {
                m_width = _width;
            }
        }
        ...
        if (range_num > 0) {
            for (var i = 0; i < range_num; i++) {
                ...
                //title处理
                var titleBegin = m_range_data_i[0][0];
                ...
                //设置title 内容
                var m_range_data_i_title = titleBegin + "~" + titleEnd;
        ...
        if (range_num > 0) {
            //获取最小值
            var rangeMin = range_data[0][0][0];
            //获取最大值
            var rangeMax = range_data[range_data.length - 1][0][1];
            if (!rangeMin) {
                rangeMin = '<' + range_data[0][0][1];
            }
            if (!rangeMax) {
                rangeMax = +range_data[range_data.length - 1][0][0] + '<';
            }
            m_inner = m_inner + "</div>" + "<div style='width:100%;height: 12px; margin-top:2px;font-size: 8px;'>" +
                "<p style='float: left;margin: 0;'>" + rangeMin + "</p>" +
                "<p style='float: right;margin: 0;'>" + rangeMax + "</p>" +
                "</div>";
        }
        m_inner = m_inner + '</div>';
        return m_inner;
